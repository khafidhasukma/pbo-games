...
public class Cellphone implements Phone{
    ...
    public void kurangiPulsa(int jumlah) throws PulsaHabisException {
        if (this.balance < jumlah) {
            throw new PulsaHabisException("Pulsa tidak cukup. Silakan isi pulsa terlebih dahulu.");
        }
        this.balance -= jumlah;
        System.out.println("Pulsa berkurang " + jumlah + ". Sisa pulsa: " + this.balance);
    }


    public void hapusContact(String nama) {
        boolean found = false;
        for (int i = 0; i < kontak.size(); i++) {
            if (kontak.get(i).getNama().equalsIgnoreCase(nama)) {
                kontak.remove(i);
